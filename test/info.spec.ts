...
function assertInfo (fmtString: string, expectProps: object) {
  expect({ ...getFormatInfo(fmtString) }, fmtString)
    .toEqual({ ...commonProps, ...expectProps });
}
...
function assertDateInfo (fmtString: string, expectProps: object) {
  expect(getFormatDateInfo(fmtString), fmtString)
...
test('numfmt.info', () => {
  ...
  expect(typeof i2, 'formatters have info objects').toBe('object');
  expect(i2, 'formatters info is what we expect').toEqual({
    type: 'number',
    isDate: false,
    isText: false,
    isPercent: false,
  ...
  expect(borked, 'info makes sense even when pattern is bogus').toEqual({
    type: 'error',
    isDate: false,
    isText: false,
    isPercent: false,
