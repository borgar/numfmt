...
export function parse_part (pattern) {
  ...
  while (s && !part_over) {
    ...
    // vulgar fractions
      ...
      if (following_num && (m[0].length > 1 || !followed_by_num)) {
        part.scale = Math.pow(0.001, m[0].length);
      }
      else {
        // regular comma
        add(m[0], tokens);
  ...
  if (part.grouping) {
    ...
    const sl = si.length;
    if (sl === 2) {
      part.group_pri = part.group_sec = si[1].length;
    }
  ...
  if (part.date && (part.int_pattern.length || part.frac_pattern.length || part.scale !== 1 || part.text)) {
