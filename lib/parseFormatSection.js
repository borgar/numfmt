...
export function parseFormatSection (inputTokens) {
  ...
  part.int_padding = patternToPadding(intPattern);
  part.man_padding = patternToPadding(manPattern);
  part.num_padding = patternToPadding(num_pat);
  part.den_padding = patternToPadding(den_pat).replace(/0$/, '1');
  part.fixed_slash = part.num_padding.includes('0') || part.den_padding.includes('0');
