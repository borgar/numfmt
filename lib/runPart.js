...
function padStart (value, pattern, nbsp = false) {
  const pd = patternToPadding(pattern, nbsp);
  return (pattern.length === 1)
    ? value || pd
    : pd.substring(0, pd.length - value.length) + value;
}

function padEnd (value, pattern, nbsp = false, fixZero = false) {
  let pd = patternToPadding(pattern, nbsp);
  if (fixZero && pd.at(-1) === '0') {
    pd = pd.slice(0, -1) + '1';
  }
  return (pd.length === 1)
    ? value || pd
    : value + pd.slice(value.length);
}
...
export function runPart (value, part, opts, l10n_) {
  ...
  // integer padding
  if (part.int_min > integer.length) {
    integer = padStart(integer, part.int_p, opts.nbsp);
  }
  // integer grouping
  if (part.grouping) {
    ...
    if (ipos > group_pri) {
      ipos -= group_pri;
      gtmp = l10n.group + integer.slice(ipos, ipos + group_pri) + gtmp;
    }
    while (ipos > group_sec) {
      ipos -= group_sec;
      gtmp = l10n.group + integer.slice(ipos, ipos + group_sec) + gtmp;
    }
  ...
  // FIXME: we can know if it needs padding by lengths, right?
  // numerator padding
  if (part.num_min > numerator.length) {
    numerator = padStart(numerator, part.num_p, opts.nbsp);
  }
  // denominator padding
  if (part.den_min > denominator.length) {
    denominator = padEnd(denominator, part.den_p, opts.nbsp, true);
  }
  // mantissa padding
  if (part.man_min > mantissa.length) {
    mantissa = padStart(mantissa, part.man_p, opts.nbsp);
  }
  // mantissa sign
  if (exp < 0) {
    mantissa_sign = '-';
  ...
  for (let ti = 0, tl = part.tokens.length; ti < tl; ti++) {
    ...
    if (tokenType === 'string') {
      ...
      if (part.int_pattern.length === 1) {
        ...
        const patt_len = part.int_p.length;
        let l = len;
        let o = counter.int;
        ...
        for (let i = 0; i < l; i++) {
          ret.push(integer[i + o] || pad(tok.num[i], opts.nbsp));
        }
        counter.int += l;
      }
    }
      ...
      if (tokenType === 'man') {
        ...
        if (mantissa.length < part.man_p.length) {
          y -= part.man_p.length - mantissa.length;
        }
      }
      ...
      if (part[tokenType + '_pattern'].length === 1) {
        ...
        for (let i = 0; i < len; i++) {
          ret.push(numstr[i + o] || pad(tok.num[i], opts.nbsp));
        }
